...
static int array_list_expand_internal(struct array_list *th, int max)
{
  void *t;
  int new_size;

  if(max < th->size) return 0;
  new_size = max(th->size << 1, max);
  if(!(t = realloc(th->array, new_size*sizeof(void*)))) return -1;
  th->array = t;
  bzero(th->array + th->size, (new_size-th->size)*sizeof(void*));
  th->size = new_size;
  return 0;
}

int
array_list_put_idx(struct array_list *t, int idx, void *data)
{
  if(array_list_expand_internal(t, idx)) return -1;
  if(t->array[idx]) t->free_fn(t->array[idx]);
  t->array[idx] = data;
  if(t->length <= idx) t->length = idx + 1;
  return 0;
}
